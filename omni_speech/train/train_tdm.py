...
class ModelArguments:
    ...
    emotion_encoder: Optional[str] = field(default=None)
    encoder_fairseq_dir: Optional[str] = field(default='/apdcephfs_nj7/share_303172353/haoyyywang/chinese-omni/emotion2vec')
    ...
    emotion_encoder_ds_rate: int = field(default=5)
    emotion_encoder_hidden_size: int = field(default=768)
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['speech_projector', 'speech_encoder', 'emotion_encoder', 'emotion_projector', 'speech_generator', 'emotion_predict_head'] # TODO 去掉就可以不加lora
...
class LazySupervisedDataset(Dataset):
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            length_list.append(sum(len(conv['content'].split()) for conv in sample['conversation']) + img_tokens)
        return length_list
    ...
    def modality_lengths(self):
        length_list = []
        for sample in self.list_data_dict:
            cur_len = sum(len(conv['content'].split()) for conv in sample['conversation'])
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'speech' in sources:
            ...
            emotion_file = self.list_data_dict[i].get("answer", speech_file)
            speech, sr = librosa.load(speech_file, sr=16000)          
            speech = torch.from_numpy(speech)
            wav = speech
            emotion_wav, sr = librosa.load(emotion_file, sr=16000)
            emotion_wav = torch.from_numpy(emotion_wav)
            emotion_wav = torch.nn.functional.layer_norm(emotion_wav, emotion_wav.shape)
            ...
            sources = preprocess_multimodal(
                copy.deepcopy(sources["conversation"]),
                self.data_args)

        data_dict = preprocess(
            sources if isinstance(sources, list) else sources['conversation'],
            self.tokenizer,
        ...
        if 'speech' in self.list_data_dict[i]:
            ...
            data_dict['wav'] = wav
            data_dict['wav_length'] = torch.LongTensor([wav.shape[0] // 320])
            data_dict['emotion_wav'] = emotion_wav
            data_dict['emotion_length'] = torch.LongTensor([emotion_wav.shape[0] // 320])

        if self.speech_gen:
            unit_file = self.list_data_dict[i]["unit"].replace('.wav', '.npy')
            # try: 
            #     unit_file = self.list_data_dict[i]["unit"]
            #     speech_token = torch.from_numpy(np.load(unit_file).squeeze())
            # except FileNotFoundError:
            #     print(f"No units found in the data. File path: {os.path.join(self.data_args.speech_folder, unit_file)}")
            # except Exception as e:
            #     print(f"{e}Error loading units. File path: {os.path.join(self.data_args.speech_folder, unit_file)}")
            # data_dict['tgt_unit'] = speech_token[torch.cat((torch.tensor([True]), speech_token[1:] != speech_token[:-1]))]
            try: 
                speech_token = torch.from_numpy(np.load(unit_file))
            except:
                speech_token, speech_token_len = self.load_tokens(unit_file)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'speech' in instances[0]:
            ...
            wavs = [instance['wav'] for instance in instances]
            wav_lengths = [instance['wav_length'] for instance in instances]
            wavs = torch.nn.utils.rnn.pad_sequence(
                wavs,
                batch_first=True,
                padding_value=0)
            emotion_wavs = [instance['emotion_wav'] for instance in instances]
            emotion_wavs = torch.nn.utils.rnn.pad_sequence(
                emotion_wavs,
                batch_first=True,
                padding_value=0)
            ...
            batch['wav'] = torch.tensor(wavs).to(torch.bfloat16)
            batch['wav_lengths'] = torch.tensor(wav_lengths)
            batch['emotion_wavs'] = torch.tensor(emotion_wavs).to(torch.bfloat16)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args, speech_gen = False, model_version = "qwen3") -> Dict:
...
def train(attn_implementation=None):
    ...
    if model_args.speech_encoder is not None:
        ...
        emotion_encoder = model.get_emotion_encoder()
        ...
        emotion_encoder = emotion_encoder.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        model.get_model().emotion_projector.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        model.get_model().emotion_predict_head.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        model.config.emotion_projector_lr = training_args.speech_projector_lr
        model.config.emotion_encoder = model_args.emotion_encoder
        model.config.encoder_fairseq_dir = model_args.encoder_fairseq_dir
    ...
    if not model_args.mm_tunable_parts:
        ...
        if "speech_projector" in tunable_parts:
            ...
            for p in model.get_model().emotion_projector.parameters():
                p.requires_grad = True
            for p in model.get_model().emotion_predict_head.parameters():
                p.requires_grad = True
