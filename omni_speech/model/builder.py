...
from omni_speech.model.speech_encoder.builder import build_speech_encoder, build_emotion_encoder
...
def load_pretrained_model(model_path, model_base=None, is_lora=False, s2s=False, load_8bit=False, load_4bit=False, device="cuda", use_flash_attn=False, **kwargs):
    ...
    if "qwen" in model_path.lower():
        print("Loading Qwen model config and use chinese-omni config")
    ...
    if is_lora:
        ...
        # if "all" in model_path.lower() or "all" in model_base.lower():
        #     from omni_speech.model.language_model.omni_speech2s_qwen2 import OmniSpeech2SConfig
        #     lora_cfg_pretrained = OmniSpeech2SConfig.from_pretrained(model_path)
        if "qwen" in model_path.lower() or "qwen" in model_base.lower():
    ...
    if hasattr(model.config, "emotion_encoder"):
        model.get_model().emotion_encoder = build_emotion_encoder(model.config)
        model.get_model().emotion_encoder.to(device=device, dtype=torch.float16)
