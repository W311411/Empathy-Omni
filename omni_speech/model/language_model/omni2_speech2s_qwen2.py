...
from omni_speech.model.language_model.omni2_speech_qwen2 import Omni2SpeechQwen2ForCausalLM
    ...
    def forward(
        ...
        if self.training:
            if self.tune_speech_generator_only:
                with torch.no_grad():
                    qwen_output = super(Omni2SpeechQwen2ForCausalLM, self).forward(
                        input_ids=input_ids,
                ...
                if torch.rand(1).item() < 0.3:
                    ...
                    loss = self.speech_generator(hidden, labels, tgt_units, txt_eos_emb, emotion)
